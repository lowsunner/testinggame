...
def build_pages(games, template_str: str, out_dir: Path, cdn_game_base: str, clean: bool, skip_existing: bool):
    ...
    for g in games:
        ...
        url_field = g.get('url') or slugify(name)
        
        # Determine the folder slug (last part of path if url is absolute)
        if url_field.startswith('/') or url_field.startswith('http://') or url_field.startswith('https://'):
            # Extract the last segment as folder name
            folder_slug = url_field.rstrip('/').split('/')[-1]
        else:
            folder_slug = url_field
        
        slugs.add(folder_slug)

        game_dir = out_dir / folder_slug
        game_dir.mkdir(parents=True, exist_ok=True)
        index_file = game_dir / 'index.html'
        if skip_existing and index_file.exists():
            print(f"skip    {folder_slug}/index.html (exists)")
            continue

        # Determine the src (iframe source)
        if 'src' in g and g['src']:
            # Explicit src provided
            src = g['src']
            if not (src.startswith('/') or src.startswith('http://') or src.startswith('https://')):
                # Relative path, prepend cdn_game_base
                src = f"{cdn_game_base.rstrip('/')}/{src}"
        elif url_field.startswith('/') or url_field.startswith('http://') or url_field.startswith('https://'):
            # url is absolute, use it as src
            src = url_field
        else:
            # Relative url, construct default src
            src = f"{cdn_game_base.rstrip('/')}/{url_field}/"
        ...
        banner = f"<!-- Generated {datetime.utcnow().isoformat()}Z by The Math Hub for '{folder_slug}'. Do not edit this file directly. -->\n"

        with index_file.open('w', encoding='utf-8') as f:
            f.write(banner + html)
        print(f"wrote   {folder_slug}/index.html")
